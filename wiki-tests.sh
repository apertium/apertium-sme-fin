...
if [ "$4" == "update" ]; then
    ...
    wget -O $TMPHTML -q http://wiki.apertium.org/wiki/Northern_Sámi_and_Finnish/$TESTTYPE
    if [[ -s $TMPHTML ]]; then mv $TMPHTML $HTML;
    else rm $TMPHTML; echo "Couldn't fetch http://wiki.apertium.org/wiki/Northern_Sámi_and_Finnish/$TESTTYPE"; fi
fi
echo "..."
